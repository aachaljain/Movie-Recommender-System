...
class TMDBMovieDetails(BaseModel):
    ...
    backdrop_url: Optional[float] = None
    genre: List[dict] = []
...
def tfidf_recommend_titles(
    ...
    """
    Returns list of (title,score) from localdf using cosine similarity on TF-IDF matrix.
    safe against ,issing  columns/rows.
    """
    global df, tfidf_matrix
    if df is None or tfidf_matrix is None:
        raise HTTPException(status_coe=500, detail="TF-IDF resources not loaded")
...
#Lifespan function replaces @app.on_event("startup")
@asynccontextmanager
